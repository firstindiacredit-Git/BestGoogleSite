...
const CategoryHome = ({ categoryType }) => {
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      ...
      if (currentUser) {
        ...
        const unsubscribeCategory = onSnapshot(categoryQuery, async (categorySnapshot) => {
          if (!categorySnapshot.empty) {
            ...
            const adminLinksUnsubscribe = onSnapshot(adminLinksQuery, (adminLinksSnapshot) => {
              ...
              const userBookmarksUnsubscribe = onSnapshot(userBookmarksQuery, (userSnapshot) => {
  ...
  const handleEdit = async () => {
    try {
      ...
      const bookmarkRef = doc(db, "users", user.uid, "bookmarks", editingBookmark.id);
  ...
  const renderSettingsMenu = () => {
    const dropdownContent = showSettings && (
      <div 
        className="fixed w-48 bg-white dark:text-white dark:bg-[#28283A] rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-[9999]"
        style={{
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-1">
              <button
                ...
                className={`p-1 rounded ${viewMode === "list" ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
              >
                List
              </button>
              <button
                onClick={() => setViewMode("grid")}
                className={`p-1 rounded ${viewMode === "grid" ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
              >
                Grid
              </button>
              <button
                onClick={() => setViewMode("cloud")}
                className={`p-1 rounded ${viewMode === "cloud" ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
              >
                Cloud
              </button>
            </div>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-1">
              <button
                ...
                className={`p-1 w-10 mx-2 rounded ${iconSize === "small" ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
              >
                S
              </button>
              <button
                onClick={() => setIconSize("medium")}
                className={`p-1 w-10 mx-2 rounded ${iconSize === "medium" ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
              >
                M
              </button>
              <button
                onClick={() => setIconSize("large")}
                className={`p-1 w-10 mx-2 rounded ${iconSize === "large" ? "bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300" : "hover:bg-gray-100 dark:hover:bg-gray-700"}`}
              >
                L
              </button>
            </div>
    ...
    return (
      <div className="relative flex justify-between  w-full">
         <button
                ...
                className="flex dark:text-white items-center rounded-lg gap-2 p-2 hover:bg-gray-100 dark:hover:bg-gray-700 text-sm"
              >
        ...
        <button
          ...
          className="p-2 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
        >
  ...
  const renderBookmarks = () => {
    ...
    const views = {
      list: {
        ...
        item: "flex items-center p-3 rounded-lg bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600",
        content: "flex items-center w-full",
        details: "flex-grow",
      },
      grid: {
        container: `grid grid-cols-${gridColumns} gap-4`,
        item: "flex flex-col items-center p-4 rounded-lg bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600",
        content: "flex flex-col items-center text-center w-full",
        details: "w-full mt-2 overflow-hidden",
      },
      cloud: {
        container: "flex flex-wrap gap-4",
        item: "flex items-center justify-center p-4 rounded-full bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600",
          ...
          >
            <div className={currentView.content}>
              ...
              <div className={currentView.details}>
                {showUrl && (
                  <div className={`${commonClasses.title} ${viewMode === 'grid' ? 'truncate text-center text-sm' : ''}`}>
                    {item.name}
                  </div>
                )}
              </div>
  ...
  return (
    <div className="bg-white dark:bg-[#28283A] rounded-lg p-4 shadow-sm isolate">
          ...
          >
            <div className="space-y-2">
              {bookmarks.map((bookmark) => (
                <div key={bookmark.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <div className="font-medium dark:text-white">{bookmark.name}</div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">{bookmark.link}</div>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setEditingBookmark({ ...editingBookmark, link: e.target.value })}
                    placeholder="Enter bookmark URL"
                  />
                </div>
              </div>
