    ...
    <List
      ...
      renderItem={(item, index) => (
        <List.Item>
          <List.Item.Meta
            title={`${index + 1}. ${item["Bank name"] || "Unknown Bank"}`}
            description={`Rank: ${item["Rank"] || "N/A"} | Total Assets: ${
              item["Total assets(2023)(US$ billion)"] || "N/A"
            } billion USD`}
          />
        </List.Item>
      )}
...
const Top100Page = () => {
      ...
      switch (category) {
        ...
        case "billionaires":
          processedData = data.map((person) => ({
            ...
            ).toFixed(1)}B, Source: ${person.source}, Country: ${
  ...
  return (
    <div className="w-[90vw] backdrop-blur-sm text-white mx-auto p-4 bg-gray-200/[var(--widget-opacity)] dark:bg-[#513a7a]/[var(--widget-opacity)] border border-gray-200 dark:border-gray-800 rounded-xl">
      <Title
        level={1}
